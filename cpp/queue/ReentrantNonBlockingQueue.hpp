...
namespace MTL::queue {
    ...
    */
    ...
    public:
    	...
    	/** this structure provides the queue TAIL and OLD_TAIL, able to be fetched, set and cmpexch simultaneously.
    	  * It is the base of the PREV-TAIL atomic strategy as described in https://.....
    	  *   - TAIL always points to the last enqueued element, but it's 'next' cannot be trusted -- the stop condition is not 'is next null?' but 'is this element the TAIL?'
    	  *   - PREV, if not null, points to an element whose 'next' is guaranteed to be trusted
          *   - when enqueueing, HEAD will only come from null after the PREV-TAIL has been set
        ...
        inline void enqueue(unsigned elementId) {
            ...
            if (currentTail != -1) {
                ...
                // if there was a HEAD (currentTail is not -1), the work is done.
                return ;
            } else {

                // at this point, HEAD was -1 -- the only way HEAD is -1 is if TAIL is also -1.
                // the thread responsible for pointing to the new list is the one that
                // create the first element of the new list -- a new list happens whenever HEAD is null
                ...
                } while (!atomicQueue.compare_exchange_strong(currentQueue, newQueue,
        ...
        inline unsigned dequeue(QueueSlot** slot) {
            ...
            do {
                ...
                if (currentQueue.head == -1) {
                    ...
                    // 'SINGLE ELEMENT' case -- attempt to null the queue so the element can be returned
                    newQueue.head = -1;
                    newQueue.tail = -1;
                    ...
                    if ( (newQueue.head = backingArray[currentQueue.head].next.load(memory_order_relaxed)) == -1 ) {
                        // if 'next' is -1, it is still being enqueued by another thread. try again...
                        continue;
                    }
                    // attempt to get the authorization to dequeue the element without attention to 'TAIL' changes
                    if (queue.head.compare_exchange_strong(currentQueue.head, newQueue.head,
                                                           memory_order_release,
                                                           memory_order_relaxed)) {
                        ...
                        // some other thread already dequeued our candidate. try again
                        continue;
                    }
                }
            } while (true);
